<template>
  <div class="sticky top-[90px] z-30">
    ...
    >
      <!-- CATEGORY -->
      <div v-for="(section, idx) in sidebarMenu" :key="idx" class="mb-2">
        <!-- TITLE -->
        <div class="bg-gradient-to-r from-orange-500 to-orange-600 px-6 py-3">
          <h3 class="text-base font-bold text-white uppercase tracking-wide text-left">
          ...
          <ul class="space-y-1">
            <li v-for="(item, itemIdx) in section.items" :key="itemIdx">
              ...
              >
                ...
                <a   :href="`/bai-viet/${getItemId(item)}`"
      ...
      <!-- CONTACT -->
      <div class="p-6 bg-[#0a203f]/90 text-center border-t border-indigo-800/50">
        <p class="text-sm text-gray-400 mb-4">Cần tư vấn ngay?</p>
...
export default {
  ...
  methods: {
    getCategoryLabel(category) {
      return category[`category_${this.lang}`] || category.category || "";
    },

    getItemTitle(item) {
      
      return item[`title_${this.lang}`] || item.title || "";
    },

     getItemId(item) {
      console.log("item in getItemTitle:", item.id);
      
      return item.id || "";
    },

    getSubItemTitle(sub) {
      return sub[`title_${this.lang}`] || sub.title || "";
    },

    async getSidebarMenu() {
      try {
        const response = await axios.post("http://192.168.51.252:3000/sidebar-menu", {
          idFun: 114,
        });
