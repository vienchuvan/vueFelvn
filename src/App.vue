...
position: relative; 
  min-height: 100vh;
  /* Đảm bảo không có overflow: auto hoặc hidden ở đây */
...
.news-page-bg::before {
  ...
  inset: 0;
  z-index: -2;
  background: linear-gradient(135deg, #1b8dd6 0%, #0f4f87 45%, #0a2342 100%);
  pointer-events: none;
}
...
.news-page-bg::after {
  ...
  z-index: -1;
  opacity: 0.18;
  background-repeat: no-repeat; /* FULL WIDTH */
  background-size: cover; /* CĂN CHUẨN NHƯ MẪU */
